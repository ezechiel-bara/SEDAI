"""
Script de génération du fichier Word pour la section 3.2 Procédure de Test
du mémoire SEDAI. Utilise python-docx.
"""
...
# ─── Titre de section ────────────────────────────────────────────────────────
titre = doc.add_heading('3.2 PROCÉDURE DE TEST', level=2)
titre.runs[0].font.color.rgb = RGBColor(0, 0, 0)

# ─── Introduction ────────────────────────────────────────────────────────────
intro = doc.add_paragraph(
    "Cette procédure de test a pour objectif de valider le fonctionnement, la fiabilité "
    "et l'efficacité du système embarqué SEDAI, en vérifiant la bonne exécution des logiques "
    "matérielles et logicielles. Elle s'assurera de la bonne communication entre le boîtier "
    "et l'application mobile, du fonctionnement de l'intelligence artificielle pour l'assistance "
    "vocale, ainsi que de la surveillance continue des données du véhicule."
)
...
protocoles = [
    {
        "titre": "Protocole 1 : Démarrage, Configuration et Connexion",
        "points": [
    ...
    {
        "titre": "Protocole 2 : Surveillance Continue et Alerte d'Anomalie (IA)",
        "points": [
    ...
    {
        "titre": "Protocole 3 : Interaction avec l'Assistant Vocal",
        "points": [
    ...
    {
        "titre": "Protocole 4 : Navigation et Historique de l'Application Mobile",
        "points": [
    ...
    {
        "titre": "Protocole 5 : Test de Résilience et de Reconnexion Automatique",
        "points": [
    ...
    {
        "titre": "Protocole 6 : Chat Textuel avec l'Intelligence Artificielle (Fonctionnalité Complémentaire)",
        "points": [
            ("Accès au Chat", "Depuis le tableau de bord de l'application, et uniquement lorsque le véhicule est à l'arrêt (vitesse = 0 km/h), appuyer sur l'icône de chat située dans la barre de navigation supérieure pour ouvrir l'interface de conversation textuelle."),
            ("Envoi d'un Message Simple", "Saisir un message de salutation (ex : « Bonjour ») dans la zone de texte et envoyer. Observer l'apparition d'un indicateur de chargement signalant que l'IA est en train de formuler sa réponse."),
...
# ─── Génération des protocoles ────────────────────────────────────────────────
for protocole in protocoles:
    # Titre du protocole (Heading 3)
    h = doc.add_heading(protocole["titre"], level=3)
    h.runs[0].font.color.rgb = RGBColor(0, 0, 0)
    h.paragraph_format.space_before = Pt(10)

    # Points à puce
    for label, contenu in protocole["points"]:
        p = doc.add_paragraph(style='List Bullet')
        run_label = p.add_run(label + " : ")
        run_label.bold = True
        run_label.font.name = 'Times New Roman'
        run_label.font.size = Pt(12)
        run_contenu = p.add_run(contenu)
        run_contenu.font.name = 'Times New Roman'
        run_contenu.font.size = Pt(12)

    doc.add_paragraph()  # Espace entre protocoles

# ─── Sauvegarde ──────────────────────────────────────────────────────────────
output_path = r"C:\Projects\SEDAI\docs\3_2_procedure_de_test.docx"
doc.save(output_path)
print(f"Fichier Word genere avec succes : {output_path}")
